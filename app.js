...
function previousPeriodKey(year=selectedYear(), month=selectedMonth()){ const d=new Date(year,month-2,1); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`; }
...
function bindEvents(){
  ...
  document.addEventListener('click', async e=>{
    ...
    if(t.dataset.fixedFreeze!==undefined){ const idx=num(t.dataset.fixedFreeze); const item=state.fixedMaster[idx]; if(item){ const currentKey=getPeriod().key, prevKey=previousPeriodKey(); const prev=fixedMonthValue(item,prevKey); item.monthly=item.monthly||{}; item.monthly[currentKey]={...(item.monthly[currentKey]||{}),amounts:{공동:num(prev.amounts?.공동),진혁:num(prev.amounts?.진혁),다혜:num(prev.amounts?.다혜)},memos:{공동:String(prev.memos?.공동||''),진혁:String(prev.memos?.진혁||''),다혜:String(prev.memos?.다혜||'')},frozenFrom:prevKey,updatedAt:new Date().toISOString()}; item.category=fixedCategory(item,currentKey); renderFixed(); await persistRemote(); showToast('전달의 금액과 메모를 현재 달에 복사했습니다.'); } }
