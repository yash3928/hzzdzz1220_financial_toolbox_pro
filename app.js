...
function mergeDefaults(data){
  ...
  merged.customBudgetCategories = Array.isArray(d.customBudgetCategories)
    ? d.customBudgetCategories
        .map(x=>({name:String(x?.name||'').trim(),scope:x?.scope==='monthly'?'monthly':'yearly'}))
        .filter(x=>x.name)
    : [];
  merged.deletedBudgetCategories = Array.isArray(d.deletedBudgetCategories)
    ? [...new Set(d.deletedBudgetCategories.map(x=>String(x||'').trim()).filter(Boolean))]
    : [];
  const customNames=merged.customBudgetCategories.map(x=>x.name);
  const validBudgetCategories=[...MONTHLY_CATEGORIES,...YEARLY_CATEGORIES,...customNames]
    .filter((c,i,a)=>a.indexOf(c)===i && !merged.deletedBudgetCategories.includes(c));
  ...
  merged.fixedMaster=merged.fixedMaster.map(it=>{
    const monthly={};
    Object.entries(it.monthly||{}).forEach(([key,rawValue])=>{
      const raw={...(rawValue||{})};
      // v1.6.41: 기존 메모는 세부 분류용으로 보존하고, 일반 메모는 별도 필드로 분리합니다.
      if(!raw.detailMemos){
        const legacyDetail={공동:'',진혁:'',다혜:'',...(raw.memos||{})};
        if(!raw.memos && raw.memo) legacyDetail[it.owner||'공동']=String(raw.memo);
        raw.detailMemos=legacyDetail;
        raw.memos={공동:'',진혁:'',다혜:''};
        raw.memo='';
      }
      monthly[key]=raw;
...
function migrateInvestSummary(existing, investments, oldAssets){
  const summary = {domestic:{principal:0,current:0,amount:0,rate:0}, overseas:{principal:0,current:0,amount:0,rate:0}, cma:{amount:0,rate:0}, ...(existing||{})};
  ['domestic','overseas'].forEach(k=>{
    const row=summary[k]||{};
    const current=num(row.current ?? row.amount);
    const principal=Object.prototype.hasOwnProperty.call(row,'principal') ? num(row.principal) : (num(row.rate)>-100 ? current/(1+num(row.rate)/100) : current);
    summary[k]={principal,current,amount:current,rate:principal?((current-principal)/principal*100):0,memo:String(row.memo||'')};
  });
  ...
  if(!hasSummary && Array.isArray(investments) && investments.length){
    ...
    summary.domestic = {principal:sums['국내주식'].p,current:sums['국내주식'].c,amount:sums['국내주식'].c, rate:sums['국내주식'].p ? ((sums['국내주식'].c - sums['국내주식'].p)/sums['국내주식'].p*100) : 0};
    summary.overseas = {principal:sums['해외주식'].p,current:sums['해외주식'].c,amount:sums['해외주식'].c, rate:sums['해외주식'].p ? ((sums['해외주식'].c - sums['해외주식'].p)/sums['해외주식'].p*100) : 0};
    summary.cma = {amount:sums['CMA'].c, rate:0};
  }
  if(!summary.domestic.current && num(oldAssets?.['국내주식'])) summary.domestic.current=summary.domestic.amount=num(oldAssets['국내주식']);
  if(!summary.overseas.current && num(oldAssets?.['해외주식'])) summary.overseas.current=summary.overseas.amount=num(oldAssets['해외주식']);
...
function fixedMonthValue(item,key=getPeriod().key){
  ...
  const detailMemos={공동:'',진혁:'',다혜:'',...(raw.detailMemos||{})};
  // 이전 버전의 단일 금액은 지정 사용자 칸으로 자동 호환합니다.
  if(!raw.amounts && num(raw.amount)) amounts[item?.owner||'공동']=num(raw.amount);
  // detailMemos 필드가 없는 구버전 메모는 세부 분류용 메모로만 해석합니다.
  if(!raw.detailMemos){
    Object.assign(detailMemos,{공동:'',진혁:'',다혜:'',...(raw.memos||{})});
    if(!raw.memos && raw.memo) detailMemos[item?.owner||'공동']=String(raw.memo);
    Object.keys(memos).forEach(owner=>memos[owner]='');
  }
  const amount=Object.values(amounts).reduce((a,v)=>a+num(v),0);
  return {...raw,budget:amount,amounts,memos,detailMemos,amount,memo:Object.values(memos).filter(Boolean).join(' ')};
}
function fixedMemoText(item,key=getPeriod().key){ const v=fixedMonthValue(item,key); return Object.values(v.detailMemos||{}).filter(Boolean).join(' '); }
...
function fixedDetailRows(item,key=getPeriod().key){
  const v=fixedMonthValue(item,key);
  return Object.entries(v.detailMemos||{}).flatMap(([owner,memo])=>{
...
function customBudgetCategoryRows(){ return Array.isArray(state.customBudgetCategories)?state.customBudgetCategories:[]; }
function allBudgetCategories(){
  const deleted=new Set(Array.isArray(state.deletedBudgetCategories)?state.deletedBudgetCategories:[]);
  return [...MONTHLY_CATEGORIES,...YEARLY_CATEGORIES,...customBudgetCategoryRows().map(x=>x.name)]
    .filter((c,i,a)=>c && a.indexOf(c)===i && !deleted.has(c));
}
function isMonthlyBudgetCategory(category){
  if(MONTHLY_CATEGORIES.includes(category)) return true;
  return customBudgetCategoryRows().some(x=>x.name===category && x.scope==='monthly');
}
function allExpenseCategories(){
  const extras=customBudgetCategoryRows().map(x=>x.name).filter(Boolean);
  return [...EXPENSE_CATEGORIES,...extras].filter((c,i,a)=>a.indexOf(c)===i);
}
function orderedBudgetCategories(){ const valid=allBudgetCategories(); const saved=Array.isArray(state.budgetOrder)?state.budgetOrder.filter(c=>valid.includes(c)):[]; return [...saved,...valid.filter(c=>!saved.includes(c))]; }
function fixedTotal(key=getPeriod().key){ return (state.fixedMaster||[]).reduce((a,f)=>a+num(fixedMonthValue(f,key).amount),0); }
function fixedInvestmentFund(key=getPeriod().key){
  const rows=(state.fixedMaster||[]).filter(f=>{ const name=String(f?.name||'').replace(/\s/g,''); return name.includes('투자자금') || name==='투자'; });
  return rows.reduce((sum,f)=>sum+num(fixedMonthValue(f,key).amount),0);
}
...
function budgetMemoValue(category,key=getPeriod().key){
  if(isMonthlyBudgetCategory(category)) return String(state.monthlyBudgetMemos?.[key]?.[category]||'');
  return String(state.budgetMemos?.[category]||'');
}
function setBudgetMemoValue(category,memo,key=getPeriod().key){
  if(isMonthlyBudgetCategory(category)){
...
function adjustmentMonth(item){
  if(item?.periodKey) return num(String(item.periodKey).split('-')[1])||selectedMonth();
  if(num(item?.month)) return num(item.month);
  const d=new Date(item?.createdAt||''); return Number.isNaN(d.getTime())?selectedMonth():d.getMonth()+1;
}
function adjustmentSignedAmount(item){ return item?.type==='삭감'?-num(item?.amount):num(item?.amount); }
function yearlyBudgetBase(category, year=selectedYear()){
  const all=(state.budgetAdjustments||[]).filter(x=>x.scope==='yearly' && x.category===category && num(x.year)===num(year));
  return Math.max(0,num(state.budgets?.[category])-all.reduce((sum,x)=>sum+adjustmentSignedAmount(x),0));
}
function yearlyBudgetValueAtMonth(category, month=selectedMonth(), year=selectedYear()){
  const base=yearlyBudgetBase(category,year);
  const applied=(state.budgetAdjustments||[]).filter(x=>x.scope==='yearly' && x.category===category && num(x.year)===num(year) && adjustmentMonth(x)<=num(month));
  return Math.max(0,base+applied.reduce((sum,x)=>sum+adjustmentSignedAmount(x),0));
}
function budgetValueForSelectedPeriod(category){
  return isMonthlyBudgetCategory(category)?monthlyBudgetValue(category):yearlyBudgetValueAtMonth(category);
}
function changeBudgetByAdjustment(item, signedAmount){
  const category=item.category;
  if(isMonthlyBudgetCategory(category)){
    const key=item.periodKey||`${num(item.year)||selectedYear()}-${String(adjustmentMonth(item)).padStart(2,'0')}`;
    state.monthlyBudgets=state.monthlyBudgets||{};
    state.monthlyBudgets[key]={...(state.monthlyBudgets[key]||{}),[category]:Math.max(0,monthlyBudgetValue(category,key)+signedAmount)};
  }else{
    state.budgets[category]=Math.max(0,num(state.budgets[category])+signedAmount);
  }
}
...
function investmentMetrics(key){ const row=state.investmentSummary?.[key]||{}; const principal=num(row.principal); const current=num(row.current ?? row.amount); const profit=current-principal; const rate=principal?profit/principal*100:0; return {principal,current,profit,rate}; }
function investAssetTotal(key=getPeriod().key){ return investmentMetrics('domestic').current+investmentMetrics('overseas').current+cmaAmountForPeriod(key); }
...
function renderHome(){
  ...
  const loanPayment=loanPaymentForKey(), investmentFund=fixedInvestmentFund();
  ...
  const standardBudgetRows=orderedBudgetCategories().filter(c=>!c.startsWith('쇼핑비(')).map(c=>{
    ...
    const b=c==='식비'?effectiveFoodBudget():c==='관리비'?mg.budget:yearlyBudgetValueAtMonth(c);
    const s=c==='관리비'?mg.actual:(isMonthlyBudgetCategory(c)?catSpent(c):annualCategorySpent(c)), bal=b-s;
    return `<tr><td>${c}</td><td>${isMonthlyBudgetCategory(c)?'월별':'연도별'}</td><td>${money(b)}</td><td>${money(s)}</td><td class="${bal<0?'minus':'plus'}">${money(bal)}</td></tr>`;
  });
  const shoppingJBudget=yearlyBudgetValueAtMonth('쇼핑비(진혁)'), shoppingDBudget=yearlyBudgetValueAtMonth('쇼핑비(다혜)');
...
function renderFixed(){
  ...
  $('#fixedAiSummary').innerHTML=detailHtml||'<p class="hint">고정지출 금액을 누르고 세부 분류용 메모에 “KT 8,000 실비 5,000”처럼 입력하면 항목별로 정리해서 표시합니다.</p>';
...
function investmentAverageRate(){
  const dm=investmentMetrics('domestic'), om=investmentMetrics('overseas');
  const principal=dm.principal+om.principal, current=dm.current+om.current;
  return principal ? (current-principal)/principal*100 : 0;
}
function renderInvest(){
  ...
  $('#investmentTable tbody').innerHTML=row('국내주식','domestic')+row('해외주식','overseas')+row('CMA','cma',true);
  const dm=investmentMetrics('domestic'), om=investmentMetrics('overseas');
  $('#investmentTable tfoot').innerHTML=`<tr class="asset-total-row"><th>합계</th><th>${money(dm.principal+om.principal+cmaAmountForPeriod())}</th><th id="investmentAssetTotal">${money(investAssetTotal())}</th><th class="${dm.profit+om.profit<0?'minus':'plus'}">${money(dm.profit+om.profit)}</th><th id="investmentAverageRate">${investmentAverageRate().toFixed(2)}%</th></tr>`;
}
function renderLoan(){
  const l={...DEFAULT_LOAN,...(state.loan||{})};
  const schedule=buildLoanSchedule();
...
function moneyMemoTarget(type,key){
  if(type==='fixed'){ const [idxRaw,ownerRaw='공동']=String(key).split(':'); const item=currentFixed()[num(idxRaw)]; if(!item) return null; const owner=['공동','진혁','다혜'].includes(ownerRaw)?ownerRaw:'공동'; const pk=getPeriod().key; const v=fixedMonthValue(item,pk); return {title:`${item.name||'고정지출'} · ${owner}`,amount:num(v.amounts?.[owner]),memo:v.memos?.[owner]||'',detailMemo:v.detailMemos?.[owner]||'',supportsDetailMemo:true,set:(amount,memo,detailMemo='')=>{item.monthly=item.monthly||{}; item.monthly[pk]={...v,amounts:{...v.amounts,[owner]:num(amount)},memos:{...v.memos,[owner]:memo},detailMemos:{...v.detailMemos,[owner]:detailMemo}}; item.category=fixedCategory(item,pk); renderFixed();}}; }
  if(type==='budget'){ const isMonthly=isMonthlyBudgetCategory(key); return {title:isMonthly?`${selectedYear()}년 ${selectedMonth()}월 ${key}`:key,amount:isMonthly?monthlyBudgetValue(key):yearlyBudgetValueAtMonth(key),memo:budgetMemoValue(key),set:(amount,memo)=>{ if(isMonthly){ setMonthlyBudgetForYear(key,amount,selectedYear(),getPeriod().key); } else state.budgets[key]=amount; setBudgetMemoValue(key,memo); state.budgetRevision=num(state.budgetRevision)+1; saveActiveYearSnapshot(); recalculateJaturi(); renderBudget(); renderHome();}}; }
...
function openMoneyMemoEditor(type,key){
  ...
  const detailWrap=$('#fixedDetailMemoWrap');
  if(detailWrap) detailWrap.hidden=!target.supportsDetailMemo;
  const detailEditor=$('#fixedDetailMemoEditor');
  if(detailEditor) detailEditor.value=target.supportsDetailMemo?(target.detailMemo||''):'';
  const generalLabel=$('#fixedGeneralMemoLabel'); if(generalLabel) generalLabel.textContent=target.supportsDetailMemo?'일반 메모':'메모';
...
function saveFixedMemoEditor(){
  ...
  target.set(num($('#fixedMemoAmount')?.value),$('#fixedMemoEditor')?.value.trim()||'',target.supportsDetailMemo?($('#fixedDetailMemoEditor')?.value.trim()||''):'');
...
function bindEvents(){
  ...
  document.addEventListener('click', async e=>{
    ...
    if(t.dataset.budgetAdjustEdit!==undefined){
      ...
      const oldSigned=item.type==='추가'?num(item.amount):-num(item.amount);
      const amount=num(prompt(`${item.category} ${item.type} 금액을 수정하세요.`,comma(item.amount))); if(amount<=0) return;
      const reason=prompt('상세 사유를 수정하세요.',item.reason||''); if(reason===null) return;
      changeBudgetByAdjustment(item,-oldSigned); item.amount=amount; item.reason=String(reason).trim(); item.updatedAt=new Date().toISOString(); changeBudgetByAdjustment(item,item.type==='추가'?amount:-amount);
      state.budgetRevision=num(state.budgetRevision)+1; saveActiveYearSnapshot(); recalculateJaturi(); await persistRemote(); renderBudget(); renderHome(); showToast('상세내역을 수정했습니다.'); return;
    }
    if(t.dataset.budgetAdjustDel!==undefined){
      const idx=(state.budgetAdjustments||[]).findIndex(x=>String(x.id)===String(t.dataset.budgetAdjustDel)); if(idx<0) return;
      const item=state.budgetAdjustments[idx]; if(!confirm(`${item.category} ${item.type} ${money(item.amount)} 내역을 삭제하고 예산을 되돌리시겠습니까?`)) return;
      const signed=item.type==='추가'?num(item.amount):-num(item.amount); changeBudgetByAdjustment(item,-signed); state.budgetAdjustments.splice(idx,1); state.budgetRevision=num(state.budgetRevision)+1; saveActiveYearSnapshot(); recalculateJaturi(); await persistRemote(); renderBudget(); renderHome(); showToast('상세내역을 삭제했습니다.'); return;
    }
    ...
    if(t.dataset.editExp){ const item=state.expenses.find(x=>x.id===t.dataset.editExp); if(item){ $('#expenseId').value=item.id; $('#expenseDate').value=item.date; $('#expensePayer').value=item.payer; $('#expenseCategory').value=item.category; updateExpenseAmountKeyboard(); $('#expenseAmount').value=comma(item.amount); $('#expenseMemo').value=item.memo||''; document.querySelector('[data-view="ledger"]').click(); setExpenseFormOpen(true); window.scrollTo({top:0,behavior:'smooth'}); }}
    if(t.dataset.delExp){
    ...
    if(t.id==='addBudgetItemBtn'){
      ...
      if(!name) return;
      const existingAll=[...MONTHLY_CATEGORIES,...YEARLY_CATEGORIES,...customBudgetCategoryRows().map(x=>x.name)];
      const deleted=Array.isArray(state.deletedBudgetCategories)?state.deletedBudgetCategories:[];
      if(existingAll.includes(name) && !deleted.includes(name)){ alert('이미 존재하는 예산 항목입니다.'); return; }
      const scopeInput=prompt('관리 방식을 입력하세요: 월별 또는 연도별','연도별');
      if(scopeInput===null) return;
      const scope=String(scopeInput).trim().startsWith('월')?'monthly':'yearly';
      state.deletedBudgetCategories=deleted.filter(x=>x!==name);
      if(!MONTHLY_CATEGORIES.includes(name) && !YEARLY_CATEGORIES.includes(name)){
        state.customBudgetCategories=customBudgetCategoryRows().filter(x=>x.name!==name);
        state.customBudgetCategories.push({name,scope});
      }
      state.budgets=state.budgets||{}; if(!Object.prototype.hasOwnProperty.call(state.budgets,name)) state.budgets[name]=0;
      state.budgetMemos=state.budgetMemos||{}; if(!Object.prototype.hasOwnProperty.call(state.budgetMemos,name)) state.budgetMemos[name]='';
      state.budgetOrder=Array.isArray(state.budgetOrder)?state.budgetOrder:[];
      if(!state.budgetOrder.includes(name)) state.budgetOrder.push(name);
      if(scope==='monthly'){
        const pk=getPeriod().key; state.monthlyBudgets=state.monthlyBudgets||{}; state.monthlyBudgets[pk]=state.monthlyBudgets[pk]||{};
        if(!Object.prototype.hasOwnProperty.call(state.monthlyBudgets[pk],name)) state.monthlyBudgets[pk][name]=0;
      }
      await persistRemote(); renderBudget(); renderLedger(); showToast('예산 항목을 추가했습니다.'); return;
    }
    if(t.dataset.budgetItemDel!==undefined){
      const name=String(t.dataset.budgetItemDel||'').trim(); if(!name) return;
      if(!confirm(`"${name}" 예산 항목을 삭제하시겠습니까?\n기존 지출내역은 삭제하지 않습니다.`)) return;
      state.deletedBudgetCategories=Array.isArray(state.deletedBudgetCategories)?state.deletedBudgetCategories:[];
      if(!state.deletedBudgetCategories.includes(name)) state.deletedBudgetCategories.push(name);
      state.customBudgetCategories=customBudgetCategoryRows().filter(x=>x.name!==name);
      state.budgetOrder=(state.budgetOrder||[]).filter(x=>x!==name);
      if(state.budgets) delete state.budgets[name];
      if(state.budgetMemos) delete state.budgetMemos[name];
      Object.values(state.monthlyBudgets||{}).forEach(row=>{ if(row) delete row[name]; });
      Object.values(state.monthlyBudgetMemos||{}).forEach(row=>{ if(row) delete row[name]; });
      state.budgetAdjustments=(state.budgetAdjustments||[]).filter(x=>x.category!==name);
      state.budgetRevision=num(state.budgetRevision)+1;
      await persistRemote(); renderBudget(); renderLedger(); renderHome(); showToast('예산 항목을 삭제했습니다.'); return;
    }
    ...
    if(t.dataset.fixedFreeze!==undefined){
      ...
      const hasPrevious=Object.values(prev.amounts||{}).some(v=>num(v)!==0) || Object.values(prev.memos||{}).some(v=>String(v||'').trim()) || Object.values(prev.detailMemos||{}).some(v=>String(v||'').trim());
      if(!hasPrevious){ alert(`${prevYear}년 ${prevMonth}월에 복사할 금액이나 메모가 없습니다.`); return; }
      const current=fixedMonthValue(item,currentKey);
      const hasCurrent=Object.values(current.amounts||{}).some(v=>num(v)!==0) || Object.values(current.memos||{}).some(v=>String(v||'').trim()) || Object.values(current.detailMemos||{}).some(v=>String(v||'').trim());
      ...
      const copiedDetailMemos={공동:String(prev.detailMemos?.공동||''),진혁:String(prev.detailMemos?.진혁||''),다혜:String(prev.detailMemos?.다혜||'')};
      item.monthly=item.monthly||{};
      item.monthly[currentKey]={
        ...(item.monthly[currentKey]||{}),
        amounts:copiedAmounts,
        memos:copiedMemos,
        detailMemos:copiedDetailMemos,
